...
| `AIPlayerPreset` | 一个可加入房间的 AI 玩家模板，引用 `ModelPreset`，并包含名称、角色提示词、策略风格和保底策略 |
...
- 快照包含 `preset_id`、`preset_version`、`model_name`、模型参数、persona、strategy 和行动上限
...
LLM API 层不暴露给模型调用；它是后端内部服务。模型只返回结构化行动，不能直接调用房间 HTTP API。
...
| `action_event_list` | 每局游戏、每轮装弹、每次回合中的每个公开行动决策与对应结果 |
